...
> [!info] ⚡絵文字は何のため？
> 稲妻の絵文字によって、エディター上でLivewireコンポーネントをすぐに見分けられます。完全に任意のものであり、好みに応じて設定で無効にできます。詳しくは[コンポーネントのドキュメント](https://livewire.laravel.com/docs/4.x/components#creating-components)を参照してください。
...
> コンポーネントには、ルートとなるHTML要素が正確に1つ必要です。この例では`<form>`要素が単一のルートです。ルートの外側に複数のルート要素やHTMLコメントがあるとエラーになります。フルページコンポーネントでは、レイアウト用の名前付きスロットをルート要素の外側に配置できます。
...
- **[コンポーネント](https://livewire.laravel.com/docs/4.x/components)** — シングルファイルとマルチファイルのコンポーネント、データの受け渡しなどを学びます。
- **[プロパティ](https://livewire.laravel.com/docs/4.x/properties)** — コンポーネントのプロパティとライフサイクルを理解します。
- **[アクション](https://livewire.laravel.com/docs/4.x/actions)** — メソッド、パラメータ、イベント処理を詳しく学びます。
- **[フォーム](https://livewire.laravel.com/docs/4.x/forms)** — リアルタイムバリデーションを含むLivewireの強力なフォーム機能を確認します。
- **[バリデーション](https://livewire.laravel.com/docs/4.x/validation)** — Livewireのバリデーション機能を習得します。
